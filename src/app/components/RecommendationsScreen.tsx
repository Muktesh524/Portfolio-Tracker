...
function RebalancingPanel({ ps }: { ps: ReturnType<typeof usePortfolioSummary> }) {
  const { computed, totalVal } = ps;

  // Sector aggregation
  const sectorMap: Record<string, number> = {};
  computed.forEach(h => { sectorMap[h.sector] = (sectorMap[h.sector] || 0) + h.currentValue; });
  const sectors = Object.entries(sectorMap)
    .map(([name, val]) => ({ name, value: val, pct: totalVal > 0 ? (val / totalVal) * 100 : 0 }))
    .sort((a, b) => b.pct - a.pct);

  // Suggest equal-weight as default target
  const equalWeight = sectors.length > 0 ? Math.round((100 / sectors.length) * 10) / 10 : 0;

  const [targets, setTargets] = useState<Record<string, number>>(() => {
    const t: Record<string, number> = {};
    sectors.forEach(s => { t[s.name] = equalWeight; });
    return t;
  });

  const totalTarget = Object.values(targets).reduce((s, v) => s + v, 0);

  const rows: TargetAlloc[] = sectors.map(s => ({
    sector:  s.name,
    current: s.pct,
    target:  targets[s.name] || 0,
    value:   s.value,
  }));

  const trades = rows.map(r => {
    const targetVal  = (r.target / 100) * totalVal;
    const delta      = targetVal - r.value;
    return { ...r, targetVal, delta };
  });
  ...
  return (
    <div style={{ background: TC.bg0, padding: '14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <SectionLabel>REBALANCING CALCULATOR — CURRENT vs TARGET</SectionLabel>
        <button
          onClick={() => {
            const t: Record<string, number> = {};
            sectors.forEach(s => { t[s.name] = equalWeight; });
      ...
      {sectors.length === 0 ? (
        <div style={{ color: TC.text5, fontSize: '10px', padding: '20px 0', textAlign: 'center' }}>
          No holdings to rebalance. Add instruments on the Holdings tab (F2).
        </div>
      ) : (
        <>
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '8px' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${TC.border}` }}>
                {['SECTOR', 'CURR ₹', 'CURR %', 'TARGET %', 'GAP', 'TRADE ₹', 'ACTION'].map((h, i) => (
                  <th key={h} style={{
                    padding: '5px 8px', color: TC.text4, fontSize: '8px', letterSpacing: '0.12em',
            ...
            <tbody>
              {trades.map((t, i) => {
                const gap = t.target - t.current;
                return (
                  <tr key={t.sector} style={{
                    background: i % 2 === 0 ? TC.bg0 : TC.bg1,
                    borderBottom: `1px solid ${TC.border2}`,
                  }}>
                    <td style={{ padding: '4px 8px', color: TC.text, fontSize: '10px', fontFamily: TC.font }}>
                      {t.sector}
                    </td>
                    <td style={{ padding: '4px 8px', color: TC.text3, fontSize: '10px', fontFamily: TC.font, textAlign: 'right' }}>
                      {fmtINR(t.value)}
                    </td>
                    <td style={{ padding: '4px 8px', color: TC.text, fontSize: '10px', fontFamily: TC.font, textAlign: 'right' }}>
                      {t.current.toFixed(1)}%
                    </td>
                    <td style={{ padding: '4px 8px', textAlign: 'right' }}>
                      <input
                        ...
                        value={t.target}
                        onChange={e => setTargets(prev => ({ ...prev, [t.sector]: parseFloat(e.target.value) || 0 }))}
                        style={{
                          width: '52px', background: TC.bg0, border: `1px solid ${TC.border}`,
                          color: TC.text, padding: '2px 4px', borderRadius: '1px',
                          fontSize: '10px', fontFamily: TC.font, textAlign: 'right', outline: 'none',
                        }}
                    ...
                    <td style={{
                      padding: '4px 8px', textAlign: 'right', fontSize: '10px', fontFamily: TC.font,
                      color: Math.abs(gap) < 1 ? TC.text5 : gap > 0 ? TC.green : TC.red,
                    }}>
                      {gap >= 0 ? '+' : ''}{gap.toFixed(1)}%
                    </td>
                    <td style={{
                      padding: '4px 8px', textAlign: 'right', fontSize: '10px', fontFamily: TC.font,
                      color: Math.abs(t.delta) < 10 ? TC.text5 : t.delta > 0 ? TC.green : TC.red,
                    }}>
                      {t.delta >= 0 ? '+' : '-'}₹{Math.abs(t.delta).toFixed(0)}
                    </td>
                    <td style={{ padding: '4px 8px', textAlign: 'right' }}>
                      {Math.abs(t.delta) < 10 ? (
                        <span style={{ color: TC.text5, fontSize: '9px' }}>—</span>
                      ) : (
                        <span style={{
                          fontSize: '8px', padding: '1px 5px', borderRadius: '1px', fontFamily: TC.font,
                          color: t.delta > 0 ? TC.green : TC.red,
                          background: (t.delta > 0 ? TC.green : TC.red) + '14',
                          border: `1px solid ${(t.delta > 0 ? TC.green : TC.red)}33`,
                        }}>
                          {t.delta > 0 ? 'BUY' : 'SELL'}
                        </span>
                      )}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              <div>
                <span style={{ color: TC.text4, fontSize: '8px', letterSpacing: '0.1em' }}>TOTAL TARGET: </span>
                <span style={{
                  color: Math.abs(totalTarget - 100) < 0.5 ? TC.green : TC.amber,
                  fontSize: '11px',
                }}>
              ...
              <div>
                <span style={{ color: TC.text4, fontSize: '8px', letterSpacing: '0.1em' }}>NET BUY: </span>
                <span style={{ color: TC.green, fontSize: '11px' }}>+{fmtINR(totalBuy)}</span>
              </div>
              <div>
                <span style={{ color: TC.text4, fontSize: '8px', letterSpacing: '0.1em' }}>NET SELL: </span>
                <span style={{ color: TC.red, fontSize: '11px' }}>-{fmtINR(totalSell)}</span>
              </div>
            </div>
            <div style={{ color: TC.text5, fontSize: '8px' }}>
              INDICATIVE ONLY — ACTUAL TRADE VALUES MAY DIFFER
            </div>
          </div>
        </>
      )}
